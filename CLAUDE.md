...
| 12a | **Computed Attributes** (collection aggregates via `GET /{resource}/computed?attributes=`, opt-in per-row values via `?computed_attributes=` / `?computedAttributes=` on index/show/trashed; declared in `collectionComputedAttributes` / `recordComputedAttributes`. Attributes may declare PARAMETERS — `{ params: [...], optionalParams: [...], using }` — supplied as `?attributes[name][param]=value`; collection callables read them on `ctx.args`, record callables as a third positional `args` object. A bare `/computed` skips required-parameter attributes rather than 403ing. Merged before the policy blacklist/whitelist; undeclared and policy-denied return the same 403; attribute and argument names are resolved with own-key lookups so a prototype key never resolves) | `controllers/global.controller.ts` (`computed`, `resolveRequestedCollectionAttributes`, `resolveRequestedComputedAttributes`, `bindAttributeSelection`), `utils/computed-attribute-spec.ts`, `utils/argument-binder.ts`, `services/resource.service.ts` (`computeCollectionAttributes`), `services/serializer.service.ts` (`resolveRecordComputed`), `interfaces/rhino-config.interface.ts` (`RecordComputedAttributeSpec`, `CollectionComputedAttributeSpec`) |
...
| 16 | **Route Groups** (incl. domain-aware groups via per-group `domain`, and per-group `tenant: false` for back-office groups with no tenant boundary) | `rhino-config.interface.ts`, `route-registration.service.ts`, `middleware/route-group.middleware.ts`, `middleware/domain-route-resolver.ts`, `middleware/tenant-route-rewrite.ts` (non-tenant prefixes are reserved segments), `utils/domain-pattern.ts`, `rhino.config.ts` (`groupHasTenantBoundary`), `services/resource-scope.service.ts` |
...
| 31 | **Named Scopes** (`?scope=<key>`, or `?scope[key][param]=value` with `static params` on the scope class, up to root `maxScopesPerRequest` (default 3) per request; gated by `namedScopes` and the policy's `permittedScopes()`) | `services/scope.service.ts` (`RhinoNamedScope`, `applyNamed` — own-property + instance guard, AND-wrap, `ctx.args`), `services/query-builder.service.ts` (`buildNamedScopes` — whitelist, policy, argument binding), `utils/argument-binder.ts` (the binding algorithm, shared with computed attributes; leaf module, imports only `RhinoException`), `services/resource.service.ts` (`findAll` applies them in URL order, fails closed when `ScopeService` absent), `rhino-config.interface.ts` (`namedScopes`/`defaultScope`), `rhino.config.ts` (`normalizeConfig` boot validation) |

...
| 31 | **Named Scopes** | Client-selectable `?scope=<key>` on index/trashed. Only keys declared in `namedScopes` are callable (own-property check — prototype keys like `constructor` are rejected with 403); each scope returns a Prisma where-fragment that Rhino **ANDs** into the query, so it can never drop the org/filter/search/soft-delete constraints. A scope class may declare `static params` so the client can fill them in with `?scope[key][param]=value`, and up to three scopes may be combined, applied in URL order. The policy's `permittedScopes()` narrows which keys this user may pick. `defaultScope` (validated at boot to be a declared key) applies when no `?scope` is sent. `show`/`update`/`destroy` prefetch stay unscoped. Fails closed (403) on an unknown name or a missing `ScopeService`. |
| 32 | **Configurable Route Key** | Choose which column matches the `:id` URL segment on member endpoints (show/update/destroy/restore/force-delete), e.g. `routeKey: 'hashId'` → `GET /api/jobs/{hash_id}`. Per-model `routeKey` beats the root config's `routeKey`; default is the primary key `id` (unchanged behavior). With a custom key the param is always treated as a string (a digit-only hash is never numified), whitelisted/`?fields` responses always keep the route-key column, and lookups stay tenant-scoped. Nested operations, FK validation, and org resolution remain primary-key based. Also available as `@RouteKey('hashId')` and blueprint `options.route_key`. |
